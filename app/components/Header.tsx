...
function UserMenu({ user, isOpen, setIsOpen }: { user: CurrentUser, isOpen: boolean, setIsOpen: (v: boolean) => void }) {
  return (
...
function CartIcon({ cartItemsCount }: { cartItemsCount: number }) {
  return (
    <Link
      to="/cart"
    ...
    >
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-2.5 5M7 13l4-2.5M17 13l-4-2.5M17 13v6a2 2 0 01-2 2H9a2 2 0 01-2-2v-6" />
      </svg>
      {cartItemsCount > 0 && (
...
function MobileMenu({ isOpen, setIsOpen, cartItemsCount }: { isOpen: boolean, setIsOpen: (v: boolean) => void, cartItemsCount: number }) {
...
function SearchOverlay({ isOpen, setIsOpen }: { isOpen: boolean, setIsOpen: (v: boolean) => void }) {
  if (!isOpen) return null
  return (
...
export function Header({ activeOrder, user }: HeaderProps) {
  ...
  return (
    <>
      <header className={`bg-white sticky top-0 z-50 transition-all duration-300 ${isScrolled ? 'shadow-soft border-b border-neutral-200/50' : 'border-b border-neutral-200'}`}>
        {/* Announcement Bar */}
        <div className="bg-gradient-to-r from-neutral-900 via-neutral-800 to-neutral-900 text-white text-center py-3">
          <p className="text-sm font-medium tracking-wide">
            ✨ Free shipping on orders over $100 • 30-day returns • 24/7 support
          </p>
        </div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="flex items-center space-x-2">
              ...
              {/* User Account */}
              {user ? (
                <UserMenu user={user} isOpen={isUserMenuOpen} setIsOpen={setIsUserMenuOpen} />
              ) : (
                <Link
                  to="/auth/login"
      ...
      <MobileMenu isOpen={isMobileMenuOpen} setIsOpen={setIsMobileMenuOpen} cartItemsCount={cartItemsCount} />
