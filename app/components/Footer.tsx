  ...
  return (
    <div>
      ...
      <ul className="space-y-4">
        {links.map(link => (
          <li key={link.to}>
            <Link to={link.to} className="text-neutral-300 hover:text-white hover:translate-x-1 transition-all duration-200 inline-block">
              {link.label}
            </Link>
          </li>
        ))}
...
function NewsletterForm({ email, setEmail, onSubmit }: { email: string, setEmail: (v: string) => void, onSubmit: (e: React.FormEvent) => void }) {
  return (
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="py-16 border-b border-neutral-700">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <NewsletterForm email={email} setEmail={setEmail} onSubmit={handleNewsletterSubmit} />
          </div>
        </div>
