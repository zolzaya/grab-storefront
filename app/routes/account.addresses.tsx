  ...
  try {
    const [addressResponse, countryResponse] = await Promise.all([
      ...
      shopApiRequest<{ availableCountries: Country[] }>(
        GET_AVAILABLE_COUNTRIES, {}, request
      )
    ])
    return {
      user,
...
function extractAddressFields(formData: FormData) {
  return {
    fullName: (formData.get("fullName") as string)?.trim(),
    company: (formData.get("company") as string)?.trim(),
    streetLine1: (formData.get("streetLine1") as string)?.trim(),
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (action === "create" || action === "update") {
    const fields = extractAddressFields(formData)
    const error = validateAddressFields(fields)
    if (error) {
      return { error, fields, action }
    }
    const input = {
      fullName: fields.fullName,
      company: fields.company || undefined,
      streetLine1: fields.streetLine1,
      ...
      defaultBillingAddress: fields.defaultBillingAddress
    }
    try {
      if (action === "create") {
        await shopApiRequest(CREATE_CUSTOMER_ADDRESS, { input }, request)
        return { success: true, message: "Address created successfully", action }
      } else {
        const addressId = formData.get("addressId") as string
        if (!addressId) return { error: "Address ID is required for update", action }
        const updateInput: UpdateAddressInput = { id: addressId, ...input }
        await shopApiRequest(UPDATE_CUSTOMER_ADDRESS, { input: updateInput }, request)
        return { success: true, message: "Address updated successfully", action }
      }
    } catch (error) {
      console.error('Address mutation error:', error)
      return { error: "Failed to save address. Please try again.", fields, action }
    }
  }

  if (action === "delete") {
    const addressId = formData.get("addressId") as string
    if (!addressId) return { error: "Address ID is required", action }
    try {
      const result = await shopApiRequest<{ deleteCustomerAddress: { success: boolean } }>(
        DELETE_CUSTOMER_ADDRESS, { id: addressId }, request
      )
      if (result.deleteCustomerAddress.success) {
        return { success: true, message: "Address deleted successfully", action }
      } else {
        return { error: "Failed to delete address", action }
      }
    } catch (error) {
      console.error('Delete address error:', error)
      return { error: "Failed to delete address. Please try again.", action }
    }
  }
...
function AddressList({
  ...
  addresses: CustomerAddress[]
  showForm: boolean
  handleEdit: (address: CustomerAddress) => void
...
function NoAddressSaved({ onAdd }: { onAdd: () => void }) {
  return (
...
export default function Addresses() {
  ...
  // Memoize addresses grid columns for performance
  const addressesGridClass = useMemo(
    () => showForm ? 'lg:col-span-1' : 'lg:col-span-3',
    [showForm]
  )
    ...
    <div className="min-h-screen bg-gradient-to-br from-neutral-50 to-white">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* Addresses List */}
          <div className={`${addressesGridClass} space-y-6 animate-fade-in-up`} style={{ animationDelay: '0.1s' }}>
            {addresses.length === 0 ? (
              <NoAddressSaved onAdd={() => setShowForm(true)} />
            ) : (
              <AddressList
                addresses={addresses}
                showForm={showForm}
        ...
        {deleteConfirm && (
          <DeleteAddressModal addressId={deleteConfirm} onCancel={() => setDeleteConfirm(null)} />
        )}
      </div>
    </div>
  )
