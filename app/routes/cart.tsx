...
function QuantityForm({ lineId, quantity, disabled }: { lineId: string, quantity: number, disabled?: boolean }) {
  return (
    <Form method="post" className="flex items-center bg-neutral-100 rounded-xl">
      ...
      <span className="px-4 py-3 font-bold text-neutral-900 min-w-[3rem] text-center">{quantity}</span>
      <button
        type="submit"
        name="quantity"
        value={quantity + 1}
...
function RemoveButton({ lineId, mobile }: { lineId: string, mobile?: boolean }) {
  return (
    <Form method="post">
      ...
      <button
        ...
        className={
          mobile
            ? "flex items-center px-6 py-3 text-error-600 hover:text-error-700 hover:bg-error-50 rounded-xl transition-all duration-200 font-medium"
            : "p-3 text-error-500 hover:text-error-700 hover:bg-error-50 rounded-xl transition-all duration-200 group"
        }
        title="Remove item from cart"
      >
        <svg className={mobile ? "w-5 h-5 mr-2" : "w-5 h-5 group-hover:scale-110 transition-transform duration-200"} fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function CartLine({ line, index }: { line: Order["lines"][0], index: number }) {
  return (
    <div key={line.id} className="p-8 hover:bg-neutral-50 transition-colors duration-200" style={{ animationDelay: `${index * 0.05}s` }}>
      <div className="flex flex-col md:flex-row md:items-center space-y-4 md:space-y-0 md:space-x-6">
        ...
        <div className="flex-1 min-w-0">
          <Link to={`/products/${line.productVariant.product.slug}`} className="group">
          ...
          <div className="flex items-center justify-between md:hidden">
            <div className="flex items-center space-x-3">
              ...
              <QuantityForm lineId={line.id} quantity={line.quantity} />
            </div>
        ...
        <div className="md:hidden flex justify-center">
          <RemoveButton lineId={line.id} mobile />
        </div>
      </div>
    </div>
  )
...
function CartSummary({ activeOrder }: { activeOrder: Order }) {
  return (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-neutral-50 to-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {/* Action Messages */}
        {actionData && 'error' in actionData && <ActionMessage error={actionData.error} />}
        {/* Cart Layout Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-2xl shadow-soft overflow-hidden">
              ...
              <div className="divide-y divide-neutral-100">
                {activeOrder.lines.map((line, index) => (
                  <CartLine key={line.id} line={line} index={index} />
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-1">
            <CartSummary activeOrder={activeOrder} />
          </div>
        </div>
      </div>
    </div>
