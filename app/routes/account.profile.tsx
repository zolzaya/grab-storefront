...
import { getCurrentUser, requireAuth, getAuthErrorMessage } from "~/lib/auth"
import type { Customer, UpdateCustomerResult } from "~/lib/types"
import { z } from "zod"
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    ...
    return { user, customer: activeCustomer }
  } catch (error) {
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  // Zod validation
  const parseResult = ProfileSchema.safeParse(fields)
  ...
  try {
    const result = await shopApiRequest<UpdateCustomerResult>(
      UPDATE_CUSTOMER,
      { input: { firstName, lastName, phoneNumber: phoneNumber || undefined } },
      request
    )

    const updateResult = result.updateCustomer
    if ('errorCode' in updateResult) {
      return { error: getAuthErrorMessage(updateResult), fields }
    }

    return { success: true, message: "Profile updated successfully" }
  } catch (error) {
    console.error('Profile update error:', error)
    return { error: "Failed to update profile. Please try again.", fields }
  }
}
...
function ProfileForm({
  ...
  const errorStyles = (hasError: boolean) =>
    `w-full px-4 py-3 border rounded-xl transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-brand-500 ${hasError ? 'border-error-300 bg-error-50' : 'border-neutral-300 hover:border-neutral-400'
    }`

  // --- Zod client validation state ---
        ...
        {clientError && (
          <div className="mb-4 bg-error-50 border-2 border-error-200 rounded-xl p-4">
            <div className="flex items-center">
              ...
              <span className="text-error-800 text-sm">{clientError}</span>
            </div>
          </div>
        )}
...
function ProfilePictureCard({ identifier }: { identifier: string }) {
  return (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-neutral-50 to-white">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {actionData?.success && <AlertSuccess message={actionData.message} />}
        {actionData?.error && <AlertError message={actionData.error} />}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            <ProfilePictureCard identifier={customer?.emailAddress} />
            <AccountInfoCard />
            <QuickActionsCard />
          </div>
        </div>
