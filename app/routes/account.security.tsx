...
import { getAuthErrorMessage, getCurrentUser, requireAuth, validateEmail, validatePassword } from "~/lib/auth"
import { shopApiRequest } from "~/lib/graphql"
import { GET_CUSTOMER_PROFILE, UPDATE_CUSTOMER_EMAIL_ADDRESS, UPDATE_CUSTOMER_PASSWORD } from "~/lib/queries"
import type { Customer, UpdateCustomerEmailResult, UpdateCustomerPasswordResult } from "~/lib/types"
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  requireAuth(user)
  const { activeCustomer } = await shopApiRequest<{ activeCustomer: Customer }>(
    GET_CUSTOMER_PROFILE, undefined, request
  )
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (action === "change-password") {
    ...
    if (!currentPassword || !newPassword || !confirmPassword) {
      return { error: "All password fields are required", action: "change-password" }
    }
    ...
    if (newPassword !== confirmPassword) {
      return { error: "New passwords do not match", action: "change-password" }
    }
    ...
    try {
      ...
      if ('errorCode' in updateResult) {
        return { error: getAuthErrorMessage(updateResult), action: "change-password" }
      }
      return { success: true, message: "Password updated successfully", action: "change-password" }
    } catch (error) {
      console.error('Password update error:', error)
      return { error: "Failed to update password. Please try again.", action: "change-password" }
    }
  }
  ...
  if (action === "change-email") {
    ...
    if (newEmailAddress === activeCustomer.emailAddress) {
      return {
...
function ErrorMessage({ error, passwordErrors }: { error: string, passwordErrors?: string[] }) {
  return (
    ...
    <div className="bg-white rounded-2xl shadow-large overflow-hidden animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
      ...
      <div className="p-8">
        {actionData?.error && actionData.action === "change-password" && (
          <ErrorMessage error={actionData.error} passwordErrors={actionData.passwordErrors} />
        )}
