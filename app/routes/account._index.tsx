...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    ...
    return { user, customer: activeCustomer, isWelcome: welcomeParam === 'true' }
  } catch (error) {
    console.error('Failed to load customer data:', error)
    return { user, customer: null, isWelcome: welcomeParam === 'true' }
  }
}

// --- Components ---

function WelcomeBanner({ show }: { show: boolean }) {
  if (!show) return null
  return (
...
function QuickActions({ totalOrders }: { totalOrders: number }) {
  return (
...
function OrderCard({ order, index }: { order: CustomerOrder, index: number }) {
  return (
    <div
...
function QuickStats({ totalOrders, recentOrders }: { totalOrders: number, recentOrders: CustomerOrder[] }) {
  return (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-neutral-50 to-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <QuickActions totalOrders={totalOrders} />
        {/* Main Content */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            <RecentOrders recentOrders={recentOrders} totalOrders={totalOrders} />
          </div>
          {/* Sidebar */}
          <div className="space-y-6 animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
            <ProfileSummary customer={customer} />
            <QuickStats totalOrders={totalOrders} recentOrders={recentOrders} />
            <HelpSupportLinks />
          </div>
        </div>
      </div>
